...
const ApproveRavnatelj = () => {
    const [registrationRequests, setRegistrationRequests] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);

    useEffect(() => {
        fetch('http://localhost:8080/admin/zahtjevi/tempRavnatelj')
    ...
    const handleApprove = (email) => {
        fetch(`http://localhost:8080/admin/zahtjevi/tempRavnatelj/${email}/odobri`, {
            method: 'POST',
        })
        .then(response => {
            if (response.ok) {
    ...
    const handleReject = (email) => {
        fetch(`http://localhost:8080/admin/zahtjevi/tempRavnatelj/${email}`, {
            method: 'DELETE',
        })
        .then(response => {
            if (response.ok) {
        ...
        <div className="homepage">
            ...
            <div className="homepage-container">
                ...
                <div className="main-content">
                    <h2>Zahtjevi za registraciju ravnatelja</h2>
                    {loading && <p>Loading...</p>}
                    {error && <p>{error}</p>}
                    {!loading && !error && (
...
export default ApproveRavnatelj;